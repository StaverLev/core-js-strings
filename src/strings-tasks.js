...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, pos) + str.slice(value.length + pos);
}
...
function reverseWords(str) {
  ...
  const reverseMapWords = words.map((word) => reverseString(word));
  return reverseMapWords.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const index = alphabet.indexOf(str[i]) + step;
    if (!alphabet.includes(str[i])) result += str[i];
    else if (
      index > alphabet.length / 2 - 1 &&
      alphabet.indexOf(str[i]) < alphabet.length / 2
    ) {
      result += alphabet[index - alphabet.length / 2];
...
function getCardId(value) {
  ...
  cardSuits.forEach((suit, index) => {
    if (suit === cardSuit) cardIndex = cardValueIndex + step * index;
  });
  return cardIndex;
}
